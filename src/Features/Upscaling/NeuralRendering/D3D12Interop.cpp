...
namespace NeuralRendering
{
		...
		ID3D12Device* sharedDevice, ID3D12CommandQueue* sharedQueue)
	{
		...
		if (FAILED(result)) return RecordFailure(result);
		if ((sharedDevice == nullptr) != (sharedQueue == nullptr))
			return RecordFailure(E_INVALIDARG);
		if (sharedDevice) {
			device12_ = sharedDevice;
			queue12_ = sharedQueue;
			logger::info("[DLSSNR] Reusing frame-generation D3D12 device and queue");
		} else {
