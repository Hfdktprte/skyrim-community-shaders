...
namespace NeuralRendering
{
	...
	public:
			...
			ID3D12Device* sharedDevice, ID3D12CommandQueue* sharedQueue)
		{
			if (failureLatched || !device || !context || eyeIndex >= eyes.size() || !color || !depth || !depthSRV || !motionVectors)
				return false;
			if (!interop.IsInitialized() && !InitializeInterop(device, context, sharedDevice, sharedQueue))
				return false;
	...
	private:
		...
		{
			if (!copyDepthGuideCS) {
				copyDepthGuideCS.Attach(static_cast<ID3D11ComputeShader*>(Util::CompileShader(
					L"Data\\Shaders\\Upscaling\\NeuralRendering\\CopyDepthGuideCS.hlsl", {}, "cs_5_0")));
				if (copyDepthGuideCS)
					Util::SetResourceName(copyDepthGuideCS.Get(), "NeuralRendering::CopyDepthGuideCS");
			}
			...
			ID3D12Device* sharedDevice = nullptr, ID3D12CommandQueue* sharedQueue = nullptr)
		{
			...
			if (FAILED(result) || !interop.Initialize(adapter.Get(), device, context, sharedDevice, sharedQueue))
